...
最初のソーススナップショットは、デスクトップアプリの `db/clickhouse.rs` からコピーされました。

ソース SHA-256: `91bf9f5fdbb89085987e6480750302e3f25cb0acb9c0734cdaf4c961366f2b79`。


このディレクトリは、`irodori.clickhouse` のマイグレーションステージングエリアです。アクティブなネイティブエントリポイントは `src/lib.rs` にあり、共有ABIヘルパーは `src/abi.rs` にあり、エンジンの動作は `src/stub.rs` または `src/driver.rs` にあります。エンジン固有の接続/クエリ/メタデータコードは、これらのスナップショットからその動作モジュールに移動すべきです。これは、コネクタのランタイム契約がデスクトップアプリに組み込まれるためです。

## マイグレーションスナップショット

| 種類 | ソース | 目的地 | SHA-256 |
|---|---|---|---|
| `desktop-db-adapter` | `apps/desktop/src-tauri/src/db/clickhouse.rs` | `native/source/irodori-table/apps/desktop/src-tauri/src/db/clickhouse.rs` | `91bf9f5fdbb89085987e6480750302e3f25cb0acb9c0734cdaf4c961366f2b79` |
